"""
Download David Tong's Cambridge books exercises.

Crawls the books page, finds courses, scans each course's
page for PDF links (exercises), and downloads them.
Features duplicate detection via progress.json.
"""
...
BASE_URL = "https://www.damtp.cam.ac.uk/user/tong/books.html"
...
def sanitize_foldername(name):
    ...
    name = re.sub(r'[<>:"/\\|?*]', "", name)
    name = name.replace("\n", " ").strip(". ")
    return name[:150].title() or "untitled"
...
async def main():
    ...
    progress_file = OUTPUT_DIR / "progress_books.json"
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        print("Fetching books course list...")
        ...
        # Scrape subjects using the books images link structure
        # e.g., <a href="books/classical.html"><img alt="[classical mechanics]" ...></a>
        book_images_div = soup.find('div', class_='book-images')
        if book_images_div:
            for a_tag in book_images_div.find_all('a', href=True):
                img = a_tag.find('img')
                if img and img.get('alt'):
                    subject_name = img['alt'].strip('[]')
        ...
        print(f"Found {len(unique_subjects)} book subjects.")
        
        for subj_name, subj_url in unique_subjects:
            print(f"\n============================================================")
            print(f"Processing Book: {subj_name.title()}")
            ...
            if not subj_html:
                print("  Failed to fetch book HTML. Skipping.")
                continue
                
            subj_soup = BeautifulSoup(subj_html, 'html.parser')
            
            # Find all PDF links on the book's page (these act as the exercises)
            pdf_links = []
            ...
            print(f"✅ Book {subj_name.title()} processed.")
