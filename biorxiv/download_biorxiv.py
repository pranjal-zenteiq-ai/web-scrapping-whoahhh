"""
Download bioRxiv preprints by subject.

Iterates through bioRxiv subjects, fetching publication metadata month-by-month
using the bioRxiv Details API, and downloads the PDFs.
...
Usage:
    python download_biorxiv.py --output-dir ./bioarxiv --max-gb 40
"""
...
START_DATE = datetime(2013, 11, 1)  # bioRxiv launch date
...
async def process_subject(session, subject, output_dir, progress, max_bytes, total_downloaded_bytes):
    ...
    # Convert subject space to underscore for API
    api_category = subj_name.lower().replace(" ", "_").replace("and", "_and_")
    # Quick fix for multiple underscores like 'pharmacology_and_toxicology'
    api_category = re.sub(r'_+', '_', api_category)
    ...
    while current_iter_date <= end_date_limit:
        ...
        while True:
            ...
            url = f"{BIORXIV_API_BASE}/{d_start}/{d_end}/{cursor}?category={api_category}"
...
async def main():
    import argparse
    parser = argparse.ArgumentParser(description="Download bioRxiv preprints by subject")
    parser.add_argument("--output-dir", default="./bioarxiv", help="Root output directory")
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        for subj in SUBJECTS:
            if subj in progress["completed_subjects"]:
            ...
            if not limit_reached and not subject_aborted:
                ...
                # Reset the subject progress to today's date so subsequent runs don't re-download from 2013
                # Actually, completing implies done until next time we decide to reset. 
                save_progress()
